import { clientKey, checkRateLimit, rateLimitResponse, cleanupRateLimits } from './_rate-limit.js';
...
export const onRequestPost = async (context) => {
  ...
  const rl = await checkRateLimit(env, clientKey(request, 'admin-code'), 5, 10);
  ...
  // Código aleatorio de 6 caracteres alfanuméricos (distingue mayúsculas)
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
  const rnd = crypto.getRandomValues(new Uint8Array(6));
  const codigo = Array.from(rnd, (b) => chars[b % chars.length]).join('');
  ...
  for (const chatId of chatIds) {
    try {
      const r = await fetch(`https://api.telegram.org/bot${env.TELEGRAM_BOT_TOKEN}/sendMessage`, {
        ...
        body: JSON.stringify({
          chat_id: chatId,
          text: `*Código de acceso al panel*\n\nTu código es: *${codigo}*\n\n⏱ Vence en 5 minutos.`,
          parse_mode: 'Markdown',
          reply_markup: {
            inline_keyboard: [
